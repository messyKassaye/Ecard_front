...
class NewCardRequestAndBuyCards extends React.Component{
    ...
    componentDidMount(){
        ...
        let PUSHER_APP_KEY = "f1cb292618c5789e5b88"
        let pusher = new Pusher(PUSHER_APP_KEY, {
        cluster: 'ap2',
        forceTLS: true
        });

        let channel = pusher.subscribe('card-request');
        channel.bind('card-request-sent',(data)=>{
    ...
    showData = (data)=>{
        ...
        cardRequests.push(data)
        this.setState(cardRequests)
    }

    componentWillReceiveProps(nextProps, nextContext) {
        if(nextProps.card_request){
        for(let i=1;i<=nextProps.card_request.length;i++){
            const {cardRequests} = this.state
            cardRequests.push(nextProps.card_request[i])
            this.setState(cardRequests)
        }
        }
    }
                        ...
                        :
                            (
                                ...
                                alignItems:'center'}}>
                                    ...
                                    <Chip size="small" label={`${this.state.cardRequests.length}`} color={'secondary'} style={{marginLeft:10,marginRight:10}} />
                                {
                                    this.state.cardRequests.length>0
                                    ?
                                        (
                                            <Button
                                            component={Link}
